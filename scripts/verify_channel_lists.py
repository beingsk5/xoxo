...
def main() -> int:
    ...
    ott = LISTS_DIR / "mib_ott_platforms.json"
    if not ott.exists():
        log.error("Missing MIB OTT list: mib_ott_platforms.json")
        return 1
    with open(ott, "r", encoding="utf-8") as f:
        ott_data = json.load(f)
    ott_total = ott_data.get("total", 0)
    print(f"MIB OTT platforms: {ott_total}")
    if ott_total == 0:
        log.error("mib_ott_platforms.json has zero platforms")
        return 1
